...
const SnackPop = ( props ) => {
		...
		if ( ajaxOptions instanceof Promise ) {
			...
			getPromise().then( ( response ) => {
				...
				setNotificationOptions( {
					...
					title: __( 'An Error Has Occurred', 'dlx-pmpro-turnstile' ),
					isDismissable: false,
					isBusy: false,
					isPersistent: true,
	...
	const getSnackbarActions = () => {
		...
		if ( notificationOptions.type === 'success' ) {
			actions.push( {
				label: __( 'Back to Top', 'dlx-pmpro-turnstile' ),
				url: '#dlx-pmpro-turnstile-admin-header',
				variant: 'link',
				className: 'dlx-pmpro-turnstile-admin__notice-action dlx-pmpro-turnstile-admin__notice-action--to-top',
			} );
		}
		return actions;
	};
	...
	const getSnackBar = () => {
		return (
			<WPSnackBar
				className={
					classnames(
						`dlx-pmpro-turnstile-snackbar dlx-pmpro-turnstile-snackbar-${ notificationOptions.type }`,
						{
							'dlx-pmpro-turnstile-snackbar-loading': isBusy,
						}
					)
				}
		...
		if ( 'critical' === notificationOptions.type ) {
			return (
				<Modal
					className={
						classnames(
							`dlx-pmpro-turnstile-modal dlx-pmpro-turnstile-modal-${ notificationOptions.type }`,
							{
								'dlx-pmpro-turnstile-modal-loading': isBusy,
							}
						)
					}
					bodyOpenClassName={ 'dlx-pmpro-turnstile-modal-body-open' }
				...
				>
					<Notice
						...
						inline={ false }
					/>
					<div className="dlx-pmpro-turnstile-modal-button-group">
						<Button
